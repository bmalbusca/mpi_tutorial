void play_non_blocking_scenario() {
  ...
  if (rank == 0) {
    ...
    // [...]
    MPI_Isend(buffer, buffer_count, MPI_INT,1,0,MPI_COMM_WORLD,&request_send);
    ...
    while (time_left > 0.0) {
      ...
      // [...]
      if(!request_finished){
        MPI_Test(&request_send, &request_finished, &status);
      }
      // 1ms left to work
      time_left -= 1000.0;
    }
    ...
    // [...]
    MPI_Isend(buffer, buffer_count, MPI_INT,1,1,MPI_COMM_WORLD,&request_send);
    ...
    // [...]
    MPI_Irecv(&buffer, buffer_count , MPI_INT, 0, 0, MPI_COMM_WORLD,&request_receive);
    ...
    // [...]
    MPI_Irecv(&buffer, buffer_count , MPI_INT, 0, 1, MPI_COMM_WORLD,&request_receive);
